...
class AggregateExamplesTest {
    ...
    fun `Social Media Post total count no windowing per user`() {
        val totalUserSocialMediaPostsTopology = AggregateExamples.buildUserSocialMediaPostsTotalCountTopology(
            inputTopicName,
            outputTopicName,
            postCreatedSerde
        )
        ...
        val outputTopic = testDriver.createOutputTopic(
            outputTopicName, Serdes.String().deserializer(),
    ...
    fun `Social Media Post with windows emit each event and total the number of posts per window by user`() {
        val totalUserSocialMediaPostsTopology = AggregateExamples.buildSocialMediaPostsWindowedTotalCountTopology(
            inputTopicName,
            outputTopicName,
            postCreatedSerde
        )
        ...
        postCreatedTopic.pipeInput(
            UUID.randomUUID().toString(),
            PostCreated(
                UUID.randomUUID().toString(), jasmine, "Drinking",
        ...
        postCreatedTopic.pipeInput(
            UUID.randomUUID().toString(),
            PostCreated(
                UUID.randomUUID().toString(), alice, "Travelling",
    ...
    fun `Social media post count emitted in suppressed window so only final window count is emitted`() {
        val totalUserSocialMediaPostsTopology = AggregateExamples.buildSocialMediaPostFinalWindowCountTopology(
            inputTopicName,
            outputTopicName,
            postCreatedSerde
        )
        ...
        postCreatedTopic.pipeInput(
            UUID.randomUUID().toString(),
            PostCreated(
                UUID.randomUUID().toString(), alice, "Close that window",
        ...
        val expectedValues = mutableListOf<KeyValue<String, Long>>(
            ...
            KeyValue(
                alice,
                2L
            ), // #Window2 Alice has a second post and a third post in the same window and it only emits the final values
